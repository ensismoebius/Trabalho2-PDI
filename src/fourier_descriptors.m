function vector= fourier_descriptors(input_img) %this function receives a img for calculating Fourier descriptors (FD)
	...
	high_coef= cell(1, 1);
	high_coef{1}= coefficients(round((rows/2)-(index_highrows/2)): round((rows/2)+(index_highrows/2)), round((columns/2)-(index_highcolumns/2)) : round((columns/2)+(index_highcolumns/2)));
